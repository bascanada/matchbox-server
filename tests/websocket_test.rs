...
async fn test_two_players_connect_to_same_lobby() {
    ...
    let (ws_stream_a, _) = connect_async(&ws_url_a).await.expect("Player A failed to connect");
    ...
    let (ws_stream_b, _) = connect_async(&ws_url_b).await.expect("Player B failed to connect");
